...
//    }
    ...
    public String resultRegistration(@ModelAttribute("person") @Valid Person person, BindingResult bindingResult){ /* принимаем из шаблона registration атрибут формы (person) указываем что модель должна валидироваться и в случае наличия ошибки они будут положены в объект bindingResult */
        ...
        return "redirect:/autentication"; // Если пользователь не является аутентифицированным(т.е. впервые прошел регистрацию) редирект на страницу person_accaunt будет перехвачен и пользователь попадет на страницу ввода логина и пароля, и сможет зарегистрироваться т.к. уже добавлен в БД пользователей.
    }
